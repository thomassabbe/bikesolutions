# This code is written at BigVision LLC. It is based on the OpenCV project. It is subject to the license terms in the
# LICENSE file found in this distribution and at http://opencv.org/license.html
...
def detect(confThreshold, nmsThreshold, imagepath):
    ...
    if args.image:
        ...
        outputFile = 'img/finished/' + args.image[:-4].replace('/', '') + '_processed.jpg'
    elif args.video:
        # Open the video file
    ...
    while cv.waitKey(1) < 0:
        ...
        # HIER METHOD CALL!!
        list = postprocess(frame, outs, confThreshold, nmsThreshold)
...
def postprocess(frame, outs, confThreshold, nmsThreshold):
    ...
    class object:
        def __init__(self, id, xcoo, ycoo, datetimenow):
            self.id = id
            self.xcoo = xcoo
            self.ycoo = ycoo
    ...
    for i in indices:
        ...
        if classes[classIds[i]] == 'bicycle':
            id = id + 1
            # (self, id, xcoo, ycoo, arr, depar, dur):
            drawPred(classIds[i], confidences[i], left, top, left + width, top + height, frame)
            # datetime object containing current date and time
            now = datetime.now()
            list.append(object(id, left + width / 2, top + height / 2, now))
    for objects in list:
        print(objects.datetimenow)
        file1.write(
            str(id) + "/ " + str(objects.xcoo) + "/ " + str(objects.ycoo) + "/ " + str(objects.datetimenow) + "\n")
